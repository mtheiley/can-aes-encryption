...
CanNetwork::CanNetwork(std::string name) {
    ...
    if(ioctl_ < 0) throw std::runtime_error("failed to manipulate CAN network interface");
    ...
    if(bind_ < 0) throw std::runtime_error("failed to bind CAN socket");
}
...
can_frame CanNetwork::read() {
    ...
    if (nbytes < 0) throw std::runtime_error("failed to read CAN bytes");
    return frame;
}
...
void CanNetwork::send(can_frame frame) {
    ...
    if(nbytes != sizeof(can_frame)) throw std::runtime_error("failed to write CAN bytes");
}
